...
DROP TABLE bddwestg.tmp093168_total_K014032022 ;
CREATE MULTISET TABLE bddwestg.tmp093168_total_K014032022 AS (
  ...
  FROM bddwestg.tmp093168_kpi14_detcpeobs_tr x0
  FULL JOIN bddwestg.tmp093168_kpi14_detcpeobs_fv x1 ON 
...
DROP TABLE bddwestg.tmp093168_dif_K014032022 ;
   ...
   y0.ser_doc,
   y0.num_doc
  FROM bddwestg.tmp093168_total_K014032022 y0
  WHERE y0.num_rucB is null
 ) WITH DATA NO PRIMARY INDEX;


DROP TABLE bddwestg.tmp093168_total_K014042022;
    CREATE MULTISET TABLE bddwestg.tmp093168_total_K014042022 AS (
       SELECT  x0.ann_ejercicio,
   x0.num_ruc,
   x0.num_doc_emisor,
   x0.cod_tip_comprob,
   ...
   x0.num_comprob,
   x1.num_ruc as num_rucB
  FROM bddwestg.tmp093168_kpi14_detcpeobs_fv x0
  FULL JOIN  bddwestg.tmp093168_kpi14_detcpeobs_mdb x1 ON
  x0.ann_ejercicio=x1.ann_ejercicio and
  x0.num_ruc=x1.num_ruc and 
...
DROP TABLE bddwestg.tmp093168_dif_K014042022 ;
 ...
 SELECT 
   ...
   y0.num_serie,
   y0.num_comprob 
 FROM bddwestg.tmp093168_total_K014042022 y0
 WHERE y0.num_rucB is null
    ) WITH DATA NO PRIMARY INDEX;
...
/********************INSERT EN TABLA FINAL***********************************/
 ...
 SELECT 
   ...
   (select coalesce(sum(cant_comp_origen),0) from bddwestg.tmp093168_kpigr14_obs_cnorigen) 
   else 0 end as cant_origen,
   coalesce(x1.cant_comp_destino1,0) as cant_destino,
   case when x0.ind_presdj=0 then 
             case when ((select count(*) from bddwestg.tmp093168_dif_K014032022)=0 and 
               (select count(*) from bddwestg.tmp093168_kpi14_detcpeobs_tr)<>0)
    then 1 else 0 end 
         end as ind_incuniv,
         case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_dif_K014032022) END as cnt_regdif,
   case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_total_K014032022 where num_ruc is null) end as cnt_regdif_do,
   case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_total_K014032022 where num_ruc=num_rucB) end as cnt_regcoinc
 FROM
 (
 ...
 SELECT 
   ...
   case when x0.ind_presdj=0  then 
    (select coalesce(sum(cant_comp_destino2),0) from bddwestg.tmp093168_kpigr14_obs_cndestino2) 
   else 0 end AS cant_destino,
   case when x0.ind_presdj=0 then 
   case when ((select count(*) from bddwestg.tmp093168_dif_K014042022)=0 and 
       (select count(*) from bddwestg.tmp093168_kpi14_detcpeobs_fv)<>0)
   then 1 else 0 end 
   end as ind_incuniv,
   case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_dif_K014042022) END as cnt_regdif,
   case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_total_K014042022 where num_ruc is null) end as cnt_regdif_do,
   case when x0.ind_presdj=0 then (select count(*) from bddwestg.tmp093168_total_K014042022 where num_ruc=num_rucB) end as cnt_regcoinc
 FROM (
