...
/********************INSERT EN TABLA FINAL***********************************/
 ...
 SELECT
   ...
   case when (select  coalesce(sum(mto_deduccion),0) from BDDWESTG.tmp093168_dif_K015012022)=0 then 1 else 0 end 
   end as ind_incuniv,
 ...
 SELECT 
   ...
   case when x0.ind_presdj=0  then (select coalesce(sum(mto_deduc_destino2),0) from BDDWESTG.tmp093168_kpigr15_cndestino2) else 0 end AS mto_destino,
    case when x0.ind_presdj=0 then 
   case when (select  coalesce(sum(mto_deduccion),0) from BDDWESTG.tmp093168_dif_K015022022)=0 then 1 else 0 end 
   end as ind_incuniv,
