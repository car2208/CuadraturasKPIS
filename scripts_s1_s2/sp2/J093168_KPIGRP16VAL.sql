/*
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_tr;
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_fv;
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_mdb;
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cnorigen;
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino1;
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino2 ;
*/
...
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_tr;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpi16_detcpeval_tr
AS
(
SELECT
 DISTINCT 
...
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_fv;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpi16_detcpeval_fv
AS
(
SELECT 
        DISTINCT 
...
DROP TABLE BDDWESTG.tmp093168_kpi16_detcpeval_mdb;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpi16_detcpeval_mdb
AS
(
SELECT
  DISTINCT 
...
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cnorigen;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpigr16_val_cnorigen AS
(
 SELECT ind_presdj,count(num_doc) as cant_comp_origen
 FROM BDDWESTG.tmp093168_kpi16_detcpeval_tr
 GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino1;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino1 AS
(
 SELECT ind_presdj,count(num_comprob) as cant_comp_destino1
 FROM BDDWESTG.tmp093168_kpi16_detcpeval_fv
 GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
DROP TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino2 ;
CREATE MULTISET TABLE BDDWESTG.tmp093168_kpigr16_val_cndestino2 AS
(
 SELECT ind_presdj,count(num_comprob) as cant_comp_destino2
 FROM BDDWESTG.tmp093168_kpi16_detcpeval_mdb
 GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
DROP TABLE BDDWESTG.tmp093168_dif_K016012022 ;
...
DROP TABLE BDDWESTG.tmp093168_dif_K016022022 ;
...
/********************INSERT EN TABLA FINAL***********************************/
 ...
 SELECT
   ...
   'K016012022' ,
   CURRENT_DATE,
   case when x0.ind_presdj=0 then (select coalesce(sum(cant_comp_origen),0) from BDDWESTG.tmp093168_kpigr16_val_cnorigen) else 0 end as cant_origen,
   coalesce(x1.cant_comp_destino1,0) as cant_destino,
   case when x0.ind_presdj=0 then 
   case when (select count(*) from BDDWESTG.tmp093168_dif_K016012022)=0 then 1 else 0 end 
   end as ind_incuniv,
   case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K016012022) END as cnt_regdif
 FROM 
 (
  select y.ind_presdj,SUM(y.cant_comp_origen) as cant_comp_origen
   from
   (
    select * from BDDWESTG.tmp093168_kpigr16_val_cnorigen
...
----------------------------------------------------------------------------------------------
 ...
 SELECT 
   ...
   case when x0.ind_presdj=0 then 
   case when (select count(*) from BDDWESTG.tmp093168_dif_K016022022)=0 then 1 else 0 end 
   end as ind_incuniv,
   case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K016022022) END as cnt_regdif
 FROM 
 (
   select y.ind_presdj,SUM(y.cant_comp_destino1) as cant_comp_destino1
   from
   (
    select * from BDDWESTG.tmp093168_kpigr16_val_cndestino1
    union all select 1,0 from (select '1' agr1) a
    union all select 0,0 from (select '0' agr0) b
   ) y group by 1
 ) x0
