...
DROP TABLE BDDWESTG.tmp093168_total_K004012022 ;
CREATE MULTISET TABLE BDDWESTG.tmp093168_total_K004012022 AS (
    ...
    FROM BDDWESTG.tmp093168_kpigr04_detcntpertr z
  ) x0
      FULL JOIN BDDWESTG.tmp093168_kpigr04_detcntperfv x1 
      ON
...
DROP TABLE BDDWESTG.tmp093168_dif_K004012022 ;
  CREATE MULTISET TABLE BDDWESTG.tmp093168_dif_K004012022 AS (
    SELECT y0.num_ruc,y0.num_docide_empl,y0.periodo
     FROM BDDWESTG.tmp093168_total_K004012022 y0
     WHERE y0.num_rucB is null
  ) WITH DATA NO PRIMARY INDEX;


DROP TABLE BDDWESTG.tmp093168_total_K004022022 ;
CREATE MULTISET TABLE BDDWESTG.tmp093168_total_K004022022 AS (
 SELECT x0.num_ruc,x0.num_doc,x0.periodo,x1.num_ruc as num_rucB 
 FROM BDDWESTG.tmp093168_kpigr04_detcntperfv x0
 FULL JOIN BDDWESTG.tmp093168_kpigr04_detcntpermdb x1 ON
 x0.num_ruc=x1.num_ruc and
...
DROP TABLE BDDWESTG.tmp093168_dif_K004022022;
  CREATE MULTISET TABLE BDDWESTG.tmp093168_dif_K004022022 AS (
   SELECT y0.num_ruc as num_ruc_trab,
      y0.num_doc as num_ruc_empl,
      y0.periodo as per_dif
  FROM BDDWESTG.tmp093168_total_K004022022 y0 
  WHERE y0.num_rucB is null
  )  WITH DATA NO PRIMARY INDEX;
...
/********************INSERT EN TABLA FINAL***********************************/
  ...
  SELECT
          ...
          case when ((select count(*) from BDDWESTG.tmp093168_dif_K004012022)=0 and
                     (select count(*) from BDDWESTG.tmp093168_kpigr04_detcntpertr)<>0)
          then 1 else 0 end 
          end as ind_incuniv,
          case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K004012022) END as cnt_regdif,
          case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_total_K004012022 where num_ruc is null) end as cnt_regdif_do,
       case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_total_K004012022 where num_ruc=num_rucB) end as cnt_regcoinc
  FROM  
  (
  ...
  SELECT '2022',
        ...
        case when x0.ind_presdj=0 then 
        case when ((select count(*) from BDDWESTG.tmp093168_dif_K004022022)=0 and
                   (select count(*) from BDDWESTG.tmp093168_kpigr04_detcntperfv)<>0)
        then 1 else 0 end end as ind_incuniv,
        case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K004022022) END as cnt_regdif,
        case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_total_K004022022 where num_ruc is null) end as cnt_regdif_do,
     case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_total_K004022022 where num_ruc=num_rucB) end as cnt_regcoinc
  FROM 
  (
...
/********************************************************************************/
  ...
  SELECT 
    ...
    INNER JOIN BDDWESTG.tmp093168_dif_K004012022 y1
    ...
    LOCK ROW FOR ACCESS
   SELECT * FROM BDDWESTG.tmp093168_dif_K004022022
    ORDER BY 1,3;
