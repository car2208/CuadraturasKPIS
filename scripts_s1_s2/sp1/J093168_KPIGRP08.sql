...
/********************INSERT EN TABLA FINAL***********************************/
  ...
  SELECT
        ...
        case when x0.ind_presdj=0 then 
        case when (select count(*) from BDDWESTG.tmp093168_dif_K008012022)=0 then 1 else 0 end 
        end as ind_incuniv,
        case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K008012022) END as cnt_regdif
  FROM 
  (
  ...
  (COD_PER,IND_PRESDJ,COD_KPI,FEC_CARGA,CNT_REGORIGEN,CNT_REGIDESTINO,IND_INCUNIV,CNT_REGDIF)
    SELECT
          ...
          case when x0.ind_presdj=0 then 
          case when (select count(*) from BDDWESTG.tmp093168_dif_K008022022)=0 then 1 else 0 end 
          end as ind_incuniv,
          case when x0.ind_presdj=0 then (select count(*) from BDDWESTG.tmp093168_dif_K008022022) END as cnt_regdif
    FROM 
    (
