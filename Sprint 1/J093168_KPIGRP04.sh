...
### $6 : Ruta Log TERADATA
### $7: Periodo :${PERIODO}
##  sh /work1/teradata/shells/093168/J093168_KPIGRP04.sh tdsunat usr_carga_prod twusr_carga_prod BDDWETB BDDWEDQ BDDWESTG BDDWELND /work1/teradata/log/093168 2022 2023-01-31
##  sh /work1/teradata/shells/093168/J093168_KPIGRP04.sh tdtp01s2 usr_carga_desa twusr_carga_desa BDDWETBD BDDWEDQD BDDWESTGD BDDWELNDD /work1/teradata/log/093168 2022 2023-01-31
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_01} AS (
    ...
    FROM ${BD_STG}.tmp093168_kpigr04_detcntpertr z
  ) x0
      FULL JOIN ${BD_STG}.tmp093168_kpigr04_detcntperfv x1 
      ON
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_02} AS (
	...
	FROM ${BD_STG}.tmp093168_kpigr04_detcntperfv x0
	FULL JOIN ${BD_STG}.tmp093168_kpigr04_detcntpermdb x1 ON
	x0.num_ruc=x1.num_ruc and
...
/********************INSERT EN TABLA FINAL***********************************/
  ...
  SELECT
          ...
          '${KPI_01}',
          CURRENT_DATE,
          case when x0.ind_presdj=0 then (select coalesce(sum(cant_per_origen),0) from ${BD_STG}.tmp093168_kpigr04_cnorigen) else 0 end as cant_origen,
          ...
          case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and
                     (select count(*) from ${BD_STG}.tmp093168_kpigr04_detcntpertr)<>0)
          then 1 else 0 end 
          end as ind_incuniv,
  ...
  (
      ...
      from
      (
        select * from ${BD_STG}.tmp093168_kpigr04_cnorigen
...
.IF ERRORCODE <> 0 THEN .GOTO error_shell;
  ...
  SELECT '${PERIODO}',
        ...
        case when x0.ind_presdj=0  then (select coalesce(sum(cant_per_destino2),0) from ${BD_STG}.tmp093168_kpigr04_cndestino2) else 0 end AS cant_destino,
        case when x0.ind_presdj=0 then 
        case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_02})=0 and
                   (select count(*) from ${BD_STG}.tmp093168_kpigr04_detcntperfv)<>0)
  ...
  (
      ...
      from
      (
        select * from ${BD_STG}.tmp093168_kpigr04_cndestino1
        union all select 1,0 from (select '1' agr1) a
        union all select 0,0 from (select '0' agr0) b
      ) y group by 1
  ) x0
