...
*/
  ...
  --Saldo a favor aplicado (hsf):
   ...
   AND hsf_tiptra = '1041'
   AND hsf_tipcta = '01'
   AND hsf_fecsaf <= CAST('2023-01-31' AS DATE FORMAT 'YYYY-MM-DD');
  ...
  --  Otros cr�ditos de ley (dbt/doc):
   ...
   AND dbt_tiptra = 1011
   AND dbt_indrec = 0
   AND dbt_fecdoc <= CAST('2023-01-31' AS DATE FORMAT 'YYYY-MM-DD')
 ...
 /******************************* MEPECO SIRAT *********************************/
  -- Pago DDJJ 0616
     ...
     AS(
        ...
        AND t03rechazado = '0'
        AND t03f_presenta <= CAST('2023-01-31' AS DATE FORMAT 'YYYY-MM-DD')
...
/************************* INICIO TABLAS ORIGEN FVIRTUAL ***************************** /
  ...
  /*INICIO PASO2 ---------------------------------------------------------------------*/
     ...
     AS(
      ...
      INNER JOIN BDDWESTG.TMP_KPI06_selecfvirtual_D_ND  b ON a.num_sec = b.num_sec
     ...
     -- b.	Cuando el contribuyente ya present� su DDJJ Anual (inserta data):
      ...
      SUBSTR(A.PERIODO,3,4)||SUBSTR(A.PERIODO,1,2) PERIODO
      FROM BDDWESTG.T5409CAS127 a 
      INNER JOIN BDDWESTG.TMP_KPI06_selecfvirtual_D b ON a.num_sec = b.num_sec;
