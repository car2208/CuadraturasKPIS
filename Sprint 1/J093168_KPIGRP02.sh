...
### $6 : Ruta Log TERADATA
### $7: Periodo :2022
##  sh /work1/teradata/shells/093168/J093168_KPIGRP02.sh tdsunat usr_carga_prod twusr_carga_prod BDDWETB BDDWEDQ BDDWESTG BDDWELND /work1/teradata/log/093168 2022 2023-01-31
##  sh /work1/teradata/shells/093168/J093168_KPIGRP02.sh tdtp01s2 usr_carga_desa twusr_carga_desa BDDWETBD BDDWEDQD BDDWESTGD BDDWELNDD /work1/teradata/log/093168 2022 2023-01-31
################################################################################
...
AND fec_presenta <= CAST('${FECHA_CORTE}' AS DATE FORMAT 'YYYY-MM-DD')
...
(
   ...
   WHERE TRIM(num_rucs) IN (SELECT TRIM(num_docide_aseg) FROM ${BD_STG}.tmp093168_kpigr2_periodos_ctaind)
)
WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr02_detcntperfv';
.IF activitycount = 0 THEN .GOTO ok 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr02_detcntpermdb';
.IF activitycount = 0 THEN .GOTO ok 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr02_cnorigen';
.IF activitycount = 0 THEN .GOTO ok 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr02_cndestino1';
.IF activitycount = 0 THEN .GOTO ok 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr02_cndestino2';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_01} AS (
	...
	FROM ( select 
	  ...
	  from ${BD_STG}.tmp093168_kpigr02_detcntpertr z
	)  x0
	FULL JOIN ${BD_STG}.tmp093168_kpigr02_detcntperfv x1 ON
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_02} AS (
	...
	FROM ${BD_STG}.tmp093168_kpigr02_detcntperfv x0
	FULL JOIN ${BD_STG}.tmp093168_kpigr02_detcntpermdb x1 ON
	x0.num_ruc=x1.num_ruc and
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	SELECT
			...
			case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and 
			           (select count(*) from ${BD_STG}.tmp093168_kpigr02_detcntpertr)<>0
			           )
	...
	SELECT '${PERIODO}',
			...
			case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_02})=0  and 
			          (select count(*) from ${BD_STG}.tmp093168_kpigr02_detcntperfv)<>0 
			          ) 
			then 1 else 0 end end as ind_incuniv,
