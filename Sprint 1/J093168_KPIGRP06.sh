...
#### ---------------------------------------------------------------------------
#### KPI06 Casilla 127 o Pagos directos de Cuarta Categoroa Pagos directos
...
### sh /work1/teradata/shells/093168/J093168_KPIGRP06.sh tdtp01s2 usr_carga_desa twusr_carga_desa BDDWEDQD BDDWESTGD BDDWELNDD BDDWETBD /work1/teradata/log/093168 2022 2023-01-31
### sh /work1/teradata/shells/093168/J093168_KPIGRP06.sh TDSUNAT usr_carga_prod twusr_carga_prod BDDWEDQ BDDWESTG BDDWELND BDDWETB /work1/teradata/log/093168 2022 2023-01-31
################################################################################
...
TBL_DOC=${BD_STG}'.DOC'
TBL_DB2=${BD_STG}'.DB2'
TBL_DDP=${BD_LND}'.DDP_RUC'
TBL_T03=${BD_WBT}'.T8593DJCAB'
TBL_T04=${BD_WBT}'.T8594DJDET'
...
/**** SIRAT PRICO ***/
	...
	--- Pago Boletas - No se considera boletas en proceso de devolucion:
	...
	--- Pago Boletas o No se considera boletas en proceso de reimputacion:
...
/******************************* MEPECO SIRAT *********************************/
	...
	AS(
		...
		SELECT DISTINCT num_ruc numruc, cod_per perpag, num_nabono nabono, cod_formul formul, num_orden norden 
		FROM ${TBL_T03} 
		WHERE cod_per IN (${CADENA_PERIODO})
		AND cod_formul IN ('0616','0116')	
		AND ind_cheqrechaz = '0'
		AND fec_presenta <= CAST('${FCH_PAGO}' AS DATE FORMAT 'YYYY-MM-DD')
	...
	AS(
    	...
    	WHERE num_nabono = nabono
    	AND cod_formul = formul
    	AND num_orden = norden
    	AND num_cas = '355'
    	AND val_cas IS NOT NULL
    	AND val_cas*1 > 0
    	AND fec_finvig=2000101
    	AND ind_deldwe='0'
...
-- Pago Boletas o No se considera boletas en proceso de compensacion:
...
-- Pago Boletas o No se considera boletas en proceso de devolucion:
...
-------------------------PAGOS DIRECTOS EN TRANSACCIONAL PRICO Y MEPECO--------------------------------------------------
	...
	AS(
		...
		FROM(
          ...
          LEFT JOIN ${BD_STG}.tmp093168_kpiperindj x1 ON x0.numruc = x1.num_ruc
		  WHERE substr(x0.numruc,1,1) <>'2' OR  x0.numruc in (select num_ruc from ${BD_STG}.tmp093168_rucs20_incluir)
	) WITH DATA NO PRIMARY INDEX ; 			
...
.label ok;
    ...
    CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_01} AS (
     SELECT x0.numruc as num_ruc_trab,x0.periodo,x0.formul,x0.norden, x1.num_ruc as num_rucB
	 FROM ${BD_STG}.tmp093168_kpigr06_detcntpertr x0
		FULL JOIN ${BD_STG}.tmp093168_kpigr06_detcntperfv x1 on
        x0.numruc=x1.num_ruc and
        x0.periodo=x1.periodo and
        x0.formul=x1.cod_formul and
        x0.norden=x1.num_ordope
	) WITH DATA NO PRIMARY INDEX;
...
.label ok;
	...
	SELECT x0.num_ruc as num_ruc_trab,x0.periodo,x0.cod_formul,x0.num_ordope, x1.num_ruc as num_rucB
	    FROM ${BD_STG}.tmp093168_kpigr06_detcntperfv x0
		FULL JOIN ${BD_STG}.tmp093168_kpigr06_detcntpermdb x1 on
        x0.num_ruc=x1.num_ruc and
        x0.periodo=x1.periodo and
        x0.cod_formul=x1.cod_formul and
        x0.num_ordope=x1.num_ordope 
    ) WITH DATA NO PRIMARY INDEX;
 ...
 DELETE FROM ${BD_DQ}.T11908DETKPITRIBINT  
  ...
  SELECT  '${PERIODO}',
         ...
         '${KPI_01}',
		  ...
		  case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and (select count(*) from ${BD_STG}.tmp093168_kpigr06_detcntpertr)<>0) then 1 else 0 end 
		  end as ind_incuniv,
		  ...
		  case when x0.ind_presdj=0 then (select count(*) from ${BD_STG}.tmp093168_total_${KPI_01} where num_ruc_trab=num_rucB) end as cnt_regcoinc
      FROM 
      (
  ...
  SELECT  '${PERIODO}',
		  ...
		  case when x0.ind_presdj=0 then 
		  case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_02})=0 and (select count(*) from ${BD_STG}.tmp093168_kpigr06_detcntperfv)<>0) then 1 else 0 end 
		  end as ind_incuniv,
		  ...
		  case when x0.ind_presdj=0 then (select count(*) from ${BD_STG}.tmp093168_total_${KPI_02} where num_ruc_trab=num_rucB) end as cnt_regcoinc
...
DROP TABLE ${BD_STG}.tmp093168_kpigr06_cndestino1;
DROP TABLE ${BD_STG}.tmp093168_kpigr06_cndestino2;
DROP TABLE ${BD_STG}.tmp093168_total_K006012022;
DROP TABLE ${BD_STG}.tmp093168_dif_K006012022 ;
DROP TABLE ${BD_STG}.tmp093168_total_K006022022;
DROP TABLE ${BD_STG}.tmp093168_dif_K006022022;
