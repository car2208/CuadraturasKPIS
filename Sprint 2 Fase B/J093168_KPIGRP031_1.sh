...
### $12 : Ruta log shell
### $13 : PERIODO
##  sh /work1/teradata/shells/093168/J093168_KPIGRP031_1.sh serverifx bdifx userifx passwordifx pathifx tdsunat usr_carga_prod twusr_carga_prod BDDWEDQ BDDWESTG BDDWETB /work1/teradata/log/093168 2022
##  sh /work1/teradata/shells/093168/J093168_KPIGRP031_1.sh serverifx bdifx userifx passwordifx pathifx tdtp01s2 usr_carga_desa twusr_carga_desa BDDWEDQD BDDWESTGD BDDWETBD /work1/teradata/log/093168 2022
################################################################################
...
plano=${path_log_TD}'/tmp093168_'${KPI_01}'_t1390f695movitf.unl'
...
###=====================================================================================================
###**********************************Cantidad de registros Informix T1390F695MOVITF**********************
###=====================================================================================================
...
select count(1) as cnt_reg from t1390f695movitf
WHERE per_dec between '${PERIODO}01' and '${PERIODO}12';
"
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr31_01_cantorigen';
.IF activitycount = 0 THEN .GOTO ok 
DROP TABLE ${BD_STG}.tmp093168_kpigr31_01_cantorigen;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;
.label ok;


CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr31_01_cantorigen
(
...
INSERT INTO  ${BD_STG}.tmp093168_kpigr31_01_cantorigen VALUES
(
:cnt_origen
);
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr31_01_cantdestino';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr31_01_cantdestino;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr31_01_cantdestino
AS
(
    SELECT COUNT(1) as cnt_destino
    from ${BD_TB}.T7334ITFMOV x0
...
/********************INSERT EN TABLA FINAL***********************************/
    ...
    SELECT 
        ...
        '${KPI_01}',
        CURRENT_DATE,
        (select sum(cnt_origen) from ${BD_STG}.tmp093168_kpigr31_01_cantorigen),---denominador
        (select sum(cnt_destino) from ${BD_STG}.tmp093168_kpigr31_01_cantdestino)--numerador
    ;
