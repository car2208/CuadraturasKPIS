...
### $7 : Ruta Log TERADATA
##  sh /work1/teradata/shells/093168/J093168_KPIGRP029.sh tdsunat usr_carga_prod twusr_carga_prod BDDWEDQ BDDWESTG BDDWELND /work1/teradata/log/093168 2022
##  sh /work1/teradata/shells/093168/J093168_KPIGRP029.sh tdtp01s2 usr_carga_desa twusr_carga_desa BDDWEDQD BDDWESTGD BDDWELNDD /work1/teradata/log/093168 2022
################################################################################
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr29_detruc_total';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr29_detruc_total;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr29_detruc_total
AS
(
SELECT  DISTINCT x0.ddp_numruc as num_ruc
FROM ${BD_LND}.ddp_ruc x0
INNER JOIN  ${BD_LND}.vfp_ruc x1 on x0.ddp_numruc=x1.vfp_numruc
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr29_detruc_ciiu4';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr29_detruc_ciiu4;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr29_detruc_ciiu4
AS (
SELECT DISTINCT x0.num_ruc from ${BD_LND}.t5667acteco x0
INNER JOIN ${BD_STG}.tmp093168_kpigr29_detruc_total x1 on x0.num_ruc=x1.num_ruc
...
(
    ...
    FROM ${BD_STG}.tmp093168_kpigr29_detruc_total x0
	FULL JOIN ${BD_STG}.tmp093168_kpigr29_detruc_ciiu4 x1
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	SELECT 
		...
		'${KPI_01}',
		CURRENT_DATE,
		(select count(*) from ${BD_STG}.tmp093168_kpigr29_detruc_total),
		(select count(*) from ${BD_STG}.tmp093168_kpigr29_detruc_ciiu4),
			case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0  and 
                  (select count(*) from ${BD_STG}.tmp093168_kpigr29_detruc_total)<>0) 
                  then 1 else 0 end,
