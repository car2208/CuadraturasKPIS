...
### $6 : Ruta Log TERADATA
### $7 : Periodo :2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP14VAL.sh tdsunat usr_carga_prod twusr_carga_prod BDDWEDQ BDDWESTG bddwelnd /work1/teradata/log/093168 2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP14VAL.sh tdtp01s2 usr_carga_desa twusr_carga_desa bddwedqd bddwestgd bddwelndd /work1/teradata/log/093168 2022
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_01} AS (
		...
		FROM ${BD_STG}.tmp093168_kpi14_detcpeval_tr x0
		FULL JOIN ${BD_STG}.tmp093168_kpi14_detcpeval_fv x1 ON 
...
.label ok;
    ...
    CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_02} AS (
       SELECT  x0.ann_ejercicio,
			...
			x0.num_comprob,
			x1.num_ruc as num_rucB
		FROM ${BD_STG}.tmp093168_kpi14_detcpeval_fv x0
		FULL JOIN  ${BD_STG}.tmp093168_kpi14_detcpeval_mdb x1 ON
		x0.ann_ejercicio=x1.ann_ejercicio and
		x0.num_ruc=x1.num_ruc and 
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	SELECT	
			...
			(select coalesce(sum(cant_comp_origen),0) from ${BD_STG}.tmp093168_kpigr14_val_cnorigen) 
			...
			case when x0.ind_presdj=0 then 
				case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and 
						   (select count(*) from ${BD_STG}.tmp093168_kpi14_detcpeval_tr)<>0)
				then 1 else 0 end 
			end as ind_incuniv,
	...
	FROM (
			...
			from
			(
				select * from ${BD_STG}.tmp093168_kpigr14_val_cnorigen
...
-----------------------------------------------------------------------------------------------
	...
	SELECT 
			...
			case when x0.ind_presdj=0  then 
				(select coalesce(sum(cant_comp_destino2),0) from ${BD_STG}.tmp093168_kpigr14_val_cndestino2) 
			else 0 end AS cant_destino,
			case when x0.ind_presdj=0 then 
			case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_02})=0 and 
			          (select count(*) from ${BD_STG}.tmp093168_kpi14_detcpeval_fv)<>0)
			then 1 else 0 end 
			end as ind_incuniv,
	...
	FROM (
			...
			from
			(
				select * from ${BD_STG}.tmp093168_kpigr14_val_cndestino1
				union all select 1,0 from (select '1' agr1) a
				union all select 0,0 from (select '0' agr0) b
			) y group by 1
	) x0
