...
/********************INSERT EN TABLA FINAL KPI27***********************************/
	...
	SELECT
			...
			(select coalesce(sum(cant_comp_origen),0) from ${BD_STG}.tmp093168_kpigr26_val_cnorigen) 
...
/***********************************************26******************************************************************************************************/	
	...
	SELECT
			...
			coalesce(x1.mto_destino1,0) as mto_destino,
				case when x0.ind_presdj=0 then 
			case when (select  coalesce(sum(mto_gravado),0) from ${BD_STG}.tmp093168_dif_${KPI_03})=0 then 1 else 0 end 
			end as ind_incuniv,
			case when x0.ind_presdj=0 then (select coalesce(sum(mto_gravado),0) from ${BD_STG}.tmp093168_dif_${KPI_03}) END as cnt_regdif
	FROM
	(
	...
	SELECT 
			...
			case when x0.ind_presdj=0 then 
			case when (select  coalesce(sum(mto_gravado),0) from ${BD_STG}.tmp093168_dif_${KPI_04})=0 then 1 else 0 end 
			end as ind_incuniv,
			case when x0.ind_presdj=0 then (select coalesce(sum(mto_gravado),0) from ${BD_STG}.tmp093168_dif_${KPI_04}) END as cnt_regdif			
	FROM 
	(
