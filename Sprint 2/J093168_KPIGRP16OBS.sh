...
### $6 : Ruta Log TERADATA
### $7 : Periodo :2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP16OBS.sh tdsunat usr_carga_prod twusr_carga_prod bddwedq bddwestg /work1/teradata/log/093168 2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP16OBS.sh tdtp01s2 usr_carga_desa twusr_carga_desa bddwedqd bddwestgd /work1/teradata/log/093168 2022
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeobs_tr';
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeobs_tr as
(
	SELECT 
		DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeobs_fv';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeobs_fv
AS
(
	SELECT
		DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeobs_mdb';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeobs_mdb
AS
(
SELECT DISTINCT 
        extract(year from fec_comprob) as ann_ejercicio,
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_obs_cnorigen';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_obs_cnorigen AS
(
	SELECT ind_presdj,count(num_doc) as cant_comp_origen
	FROM ${BD_STG}.tmp093168_kpi16_detcpeobs_tr
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_obs_cndestino1';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_obs_cndestino1 AS
(
	SELECT ind_presdj,count(num_comprob) as cant_comp_destino1
	FROM ${BD_STG}.tmp093168_kpi16_detcpeobs_fv
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_obs_cndestino2';
.IF activitycount = 0 THEN .GOTO ok 
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_obs_cndestino2 AS
(
	SELECT ind_presdj,count(num_comprob) as cant_comp_destino2
	FROM ${BD_STG}.tmp093168_kpi16_detcpeobs_mdb
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	FROM
		(
			SELECT 
			       x0.ind_presdj,
			 	   case when x0.ind_presdj=0 then (select coalesce(sum(cant_comp_origen),0) from ${BD_STG}.tmp093168_kpigr16_obs_cnorigen) else 0 end as cant_origen,
			...
			(
				...
				from
					(
						select * from ${BD_STG}.tmp093168_kpigr16_obs_cnorigen
						union all select 1,0 from (select '1' agr1) a
						union all select 0,0 from (select '0' agr0) b
					) y group by 1
			) x0
			LEFT JOIN ${BD_STG}.tmp093168_kpigr16_obs_cndestino1 x1 
	...
	FROM
		(
			SELECT x0.ind_presdj,
			       ...
			       case when x0.ind_presdj=0  then (select coalesce(sum(cant_comp_destino2),0) from ${BD_STG}.tmp093168_kpigr16_obs_cndestino2) else 0 end AS cant_destino
			FROM
			(
				select y.ind_presdj,SUM(y.cant_comp_destino1) as cant_comp_destino1
					from
					(
						select * from ${BD_STG}.tmp093168_kpigr16_obs_cndestino1
						union all select 1,0 from (select '1' agr1) a
						union all select 0,0 from (select '0' agr0) b
					) y group by 1
			) x0
			LEFT JOIN ${BD_STG}.tmp093168_kpigr16_obs_cndestino2 x1 
			ON x0.ind_presdj=x1.ind_presdj
		) z
	GROUP BY 1,2,3,4
	;
