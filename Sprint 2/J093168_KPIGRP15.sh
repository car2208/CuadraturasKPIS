...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi15_detcpeval_tr';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_tr;
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_tr
AS
(
SELECT 
	DISTINCT
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi15_detcpeval_fv';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_fv;

.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_fv
AS
(
SELECT 
        DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi15_detcpeval_mdb';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_mdb;

.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi15_detcpeval_mdb
AS
(
SELECT
		DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr15_cnorigen';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr15_cnorigen;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr15_cnorigen AS
(
	SELECT ind_presdj,sum(mto_deduccion_fin) as mto_deduc_origen
	FROM ${BD_STG}.tmp093168_kpi15_detcpeval_tr
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr15_cndestino1';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr15_cndestino1;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr15_cndestino1 AS
(
	SELECT ind_presdj,sum(mto_deduccion) as mto_deduc_destino1
	FROM ${BD_STG}.tmp093168_kpi15_detcpeval_fv
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr15_cndestino2';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr15_cndestino2;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr15_cndestino2 AS
(
	SELECT ind_presdj,sum(mto_deduccion) as mto_deduc_destino2
	FROM ${BD_STG}.tmp093168_kpi15_detcpeval_mdb
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	SELECT
			...
			case when ((select  coalesce(sum(mto_deduccion),0) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and
			 		   (select count(*) from ${BD_STG}.tmp093168_kpi15_detcpeval_tr)<>0
					  )
			then 1 else 0 end 
			end as ind_incuniv,
...
--------------------------------------------------------------------------------------
	...
	SELECT 
			...
			case when x0.ind_presdj=0  then (select coalesce(sum(mto_deduc_destino2),0) from ${BD_STG}.tmp093168_kpigr15_cndestino2) else 0 end AS mto_destino,
				case when x0.ind_presdj=0 then 
			case when ((select  coalesce(sum(mto_deduccion),0) from ${BD_STG}.tmp093168_dif_${KPI_02})=0 and
					   (select count(*) from ${BD_STG}.tmp093168_kpi15_detcpeval_fv)<>0)
			then 1 else 0 end 
			end as ind_incuniv,
