...
### $6 : Ruta Log TERADATA
### $7 : Periodo :2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP16VAL.sh tdsunat usr_carga_prod twusr_carga_prod bddwedq bddwestg /work1/teradata/log/093168 2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP16VAL.sh tdtp01s2 usr_carga_desa twusr_carga_desa bddwedqd bddwestgd /work1/teradata/log/093168 2022
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeval_tr';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_tr;

.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_tr
AS
(
SELECT
	DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeval_fv';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_fv;

.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_fv
AS
(
SELECT 
        DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpi16_detcpeval_mdb';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_mdb;

.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpi16_detcpeval_mdb
AS
(
SELECT
		DISTINCT 
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_val_cnorigen';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr16_val_cnorigen;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_val_cnorigen AS
(
	SELECT ind_presdj,count(num_doc) as cant_comp_origen
	FROM ${BD_STG}.tmp093168_kpi16_detcpeval_tr
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_val_cndestino1';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr16_val_cndestino1;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_val_cndestino1 AS
(
	SELECT ind_presdj,count(num_comprob) as cant_comp_destino1
	FROM ${BD_STG}.tmp093168_kpi16_detcpeval_fv
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
SELECT 1 FROM  dbc.TablesV WHERE databasename = '${BD_STG}' AND TableName = 'tmp093168_kpigr16_val_cndestino2';
.IF activitycount = 0 THEN .GOTO ok 

DROP TABLE ${BD_STG}.tmp093168_kpigr16_val_cndestino2	;
.IF ERRORCODE <> 0 THEN .GOTO error_shell;

.label ok;

CREATE MULTISET TABLE ${BD_STG}.tmp093168_kpigr16_val_cndestino2 AS
(
	SELECT ind_presdj,count(num_comprob) as cant_comp_destino2
	FROM ${BD_STG}.tmp093168_kpi16_detcpeval_mdb
	GROUP BY 1
) WITH DATA NO PRIMARY INDEX;
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	FROM
		(
			SELECT
			       x0.ind_presdj,
				   case when x0.ind_presdj=0 then (select coalesce(sum(cant_comp_origen),0) from ${BD_STG}.tmp093168_kpigr16_val_cnorigen) else 0 end as cant_origen,
			...
			(
				select y.ind_presdj,SUM(y.cant_comp_origen) as cant_comp_origen
					from
					(
						select * from ${BD_STG}.tmp093168_kpigr16_val_cnorigen
						union all select 1,0 from (select '1' agr1) a
						union all select 0,0 from (select '0' agr0) b
					) y group by 1
			) x0
			LEFT JOIN ${BD_STG}.tmp093168_kpigr16_val_cndestino1 x1 
		...
		(
			SELECT x0.ind_presdj,
			       x0.cant_comp_destino1 AS cant_origen,
			        case when x0.ind_presdj=0  then (select sum(cant_comp_destino2) from ${BD_STG}.tmp093168_kpigr16_val_cndestino2) else 0 end AS cant_destino
			FROM 
			(
					select y.ind_presdj,SUM(y.cant_comp_destino1) as cant_comp_destino1
					from
					(
						select * from ${BD_STG}.tmp093168_kpigr16_val_cndestino1
						union all select 1,0 from (select '1' agr1) a
						union all select 0,0 from (select '0' agr0) b
					) y group by 1
			) x0
			LEFT JOIN ${BD_STG}.tmp093168_kpigr16_val_cndestino2 x1 
			ON x0.ind_presdj=x1.ind_presdj
		) z
	GROUP BY 1,2,3,4
	;
