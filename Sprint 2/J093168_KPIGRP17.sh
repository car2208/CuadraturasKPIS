...
### $6 : Ruta Log TERADATA
### $7 : Periodo :2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP17.sh tdsunat usr_carga_prod twusr_carga_prod bddwedq bddwestg bddwelnd /work1/teradata/log/093168 2022
### sh /work1/teradata/shells/093168/J093168_KPIGRP17.sh tdtp01s2 usr_carga_desa twusr_carga_desa bddwedqd bddwestgd bddwelndd /work1/teradata/log/093168 2022
...
.label ok;
    ...
    CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_01} AS (
     SELECT x0.ann_ejercicio,x0.num_ruc,x0.num_ruc_emisor,x0.cod_tip_doc,x0.ser_doc,x0.num_doc,x0.mto_deduccion_fin, x1.num_ruc as num_rucB, x1.mto_deduccion as mto_deduccionB
	FROM ${BD_STG}.tmp093168_kpi17_detcpeval_tr x0
		FULL JOIN ${BD_STG}.tmp093168_kpi17_detcpeval_fv x1 on
		...
		x0.mto_deduccion_fin=x1.mto_deduccion
	) WITH DATA NO PRIMARY INDEX;
...
CREATE MULTISET TABLE ${BD_STG}.tmp093168_total_${KPI_02} AS (
	SELECT DISTINCT 
		x0.ann_ejercicio,x0.num_ruc,x0.num_doc_emisor num_ruc_emisor,x0.cod_tip_comprob,x0.num_serie,x0.num_comprob,x0.mto_deduccion, x1.num_ruc as num_rucB, x1.mto_deduccion as mto_deduccionB
	FROM ${BD_STG}.tmp093168_kpi17_detcpeval_fv x0
		FULL JOIN ${BD_STG}.tmp093168_kpi17_detcpeval_mdb x1 on
		x0.ann_ejercicio=x1.ann_ejercicio and
		x0.num_ruc=x1.num_ruc and
...
/********************INSERT EN TABLA FINAL***********************************/
	...
	SELECT '${PERIODO}',
		  ...
		  case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_01})=0 and (select count(*) from ${BD_STG}.tmp093168_kpi17_detcpeval_tr)<>0) then 1 else 0 end 
		  end as ind_incuniv,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccion_fin),0) from ${BD_STG}.tmp093168_dif_${KPI_01}) END as mto_regdif_od,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccionB),0) from ${BD_STG}.tmp093168_total_${KPI_01} where num_ruc is null) end as mto_regdif_do ,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccion_fin),0) from ${BD_STG}.tmp093168_total_${KPI_01} where num_ruc=num_rucB) end as mto_regcoinc
			FROM 
			(
	...
	SELECT  '${PERIODO}',
		  ...
		  case when x0.ind_presdj=0 then 
		  case when ((select count(*) from ${BD_STG}.tmp093168_dif_${KPI_02})=0 and (select count(*) from ${BD_STG}.tmp093168_kpi17_detcpeval_fv)<>0) then 1 else 0 end 
		  end as ind_incuniv,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccion),0) from ${BD_STG}.tmp093168_dif_${KPI_02}) END as mto_regdif_od,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccionB),0) from ${BD_STG}.tmp093168_total_${KPI_02} where num_ruc is null) end as mto_regdif_do ,
		  case when x0.ind_presdj=0 then (select coalesce(sum(mto_deduccion),0) from ${BD_STG}.tmp093168_total_${KPI_02} where num_ruc=num_rucB) end as mto_regcoinc
...
DROP TABLE ${BD_STG}.tmp093168_kpigr17_cndestino1;
DROP TABLE ${BD_STG}.tmp093168_kpigr17_cndestino2;
DROP TABLE ${BD_STG}.tmp093168_total_K017012022  ;
DROP TABLE ${BD_STG}.tmp093168_dif_K017012022    ;
DROP TABLE ${BD_STG}.tmp093168_total_K017022022  ;
DROP TABLE ${BD_STG}.tmp093168_dif_K017022022;
