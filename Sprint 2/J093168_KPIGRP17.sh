...
/********************INSERT EN TABLA FINAL***********************************/
	...
	FROM
		(
			SELECT
			       x0.ind_presdj,
			      case when x0.ind_presdj=0 then (select sum(mto_deduc_origen) from ${BD_STG}.tmp093168_kpigr17_cnorigen) else 0 end as mto_origen,
			       coalesce(x1.mto_deduc_destino1,0) as mto_destino
			FROM ${BD_STG}.tmp093168_kpigr17_cnorigen x0
	...
	FROM
		(
			SELECT x0.ind_presdj,
			       ...
			       case when x0.ind_presdj=0  then (select sum(mto_deduc_destino2) from ${BD_STG}.tmp093168_kpigr17_cndestino2) else 0 end AS mto_destino
			FROM ${BD_STG}.tmp093168_kpigr17_cndestino1 x0
